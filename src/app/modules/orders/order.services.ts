...
const createOrderIntoDB = async (
  ...
  try {
    ...
    for (const product of payload.products) {
      const productInDB = await Product.findById(product.bicycle).session(
        session,
      );
      if (!productInDB) {
        throw new AppError(
          StatusCodes.NOT_FOUND,
          `Product with ID ${product.bicycle} not found`,
        );
      }
      ...
      if (productInDB.stock < product.quantity) {
        throw new AppError(
          ...
          `Insufficient stock for ${productInDB.name}`,
        );
      }
    }
...
const getSalesDashboard = async () => {
  ...
  const productSalesMap: Record<
    string,
    { name: string; unitsSold: number; imageUrl: string; model: string }
  > = {};
